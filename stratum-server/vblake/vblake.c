...
static uint64_t bytesToLong(char bytes[8])
{
    uint64_t result =
...
static void compress(uint64_t h[], char *b)
{
    uint64_t v[16];
    ...
    for (int i = 0; i < 16; i++) {
        B2B_G(v, 0, 4, 8, 12, m[sigma[i][1]], m[sigma[i][0]], vBlake_c[sigma[i][1]],
                vBlake_c[sigma[i][0]]);

        B2B_G(v, 1, 5, 9, 13, m[sigma[i][3]], m[sigma[i][2]], vBlake_c[sigma[i][3]],
                vBlake_c[sigma[i][2]]);

        B2B_G(v, 2, 6, 10, 14, m[sigma[i][5]], m[sigma[i][4]], vBlake_c[sigma[i][5]],
                vBlake_c[sigma[i][4]]);

        B2B_G(v, 3, 7, 11, 15, m[sigma[i][7]], m[sigma[i][6]], vBlake_c[sigma[i][7]],
                vBlake_c[sigma[i][6]]);

        B2B_G(v, 0, 5, 10, 15, m[sigma[i][9]], m[sigma[i][8]], vBlake_c[sigma[i][9]],
                vBlake_c[sigma[i][8]]);

        B2B_G(v, 1, 6, 11, 12, m[sigma[i][11]], m[sigma[i][10]], vBlake_c[sigma[i][11]],
                vBlake_c[sigma[i][10]]);

        B2B_G(v, 2, 7, 8, 13, m[sigma[i][13]], m[sigma[i][12]], vBlake_c[sigma[i][13]],
                vBlake_c[sigma[i][12]]);

        B2B_G(v, 3, 4, 9, 14, m[sigma[i][15]], m[sigma[i][14]], vBlake_c[sigma[i][15]],
                vBlake_c[sigma[i][14]]);
    }
...
static void recombineB2Bh(uint64_t h[], char *output)
{
    for (int i = 0; i < 3; i++) {
        output[i * 8 + 0] = (char)(h[i] >> 0);
        output[i * 8 + 1] = (char)(h[i] >> 8);
        output[i * 8 + 2] = (char)(h[i] >> 16);
        output[i * 8 + 3] = (char)(h[i] >> 24);
        output[i * 8 + 4] = (char)(h[i] >> 32);
        output[i * 8 + 5] = (char)(h[i] >> 40);
        output[i * 8 + 6] = (char)(h[i] >> 48);
        output[i * 8 + 7] = (char)(h[i] >> 56);
    }
}
