...
def hex_to_int32(hex):
    return struct.unpack(">i", binascii.unhexlify(hex))[0]

def hex_to_int64(hex):
    return struct.unpack(">q", binascii.unhexlify(hex))[0]

def uint192_from_str_be(s):
    return int(binascii.hexlify(s[:24]), 16)
